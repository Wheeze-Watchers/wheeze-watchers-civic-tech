...
export default function () {
  ...
  useEffect(() => {
    const fetchResources = async () => {
      ...
      const validResources = response.filter(
  ...
  const handleSubmit = async (e) => {
    ...
    if (currentUser && !usedUrl.has(newUrl)) {
      const response = await fetchHandler(
        "/api/resources/",
        getPostOptions({ user_id: currentUser.id, url: newUrl })
      );
      if (response[0] === null) {
        ...
        <div className="modal-content">
          <div className="box">
            <form aria-labelledby="resource-form">
              ...
              />
              <button
                className="button medium-blue-slate has-text-weight-semibold has-text-white"
